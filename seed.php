...
// 3) Faculty (more classes per faculty)
$facultyData = [
...
// 5) Students (more per section)
$students = [];
$ids = 1;
foreach ($sections as $sec) {
    for ($i=1; $i<=8; $i++) { // 8 students per section
        $sid = 'S' . str_pad($ids, 3, '0', STR_PAD_LEFT);
        $name = "Student$ids";
        $pwd = 'stu123';
        $students[] = [$sid, $name, $pwd, $sec[0]];
        $ids++;
    }
}
...
// 7) Assigned Classes (faculty teaches multiple sections)
$assigned = [
    ['C201','2CSE1','F001'],
    ['C202','2CSE1','F001'],
    ['C203','2CSE1','F001'],
...
// 8) Marks (every student has marks for all courses of their year)
$sectionYear = [];
...
echo "<p style='color:green'>✅ Rich seeding complete. Check phpMyAdmin or your app.</p>";
echo "<p>Remove/rename <strong>seed.php</strong> after running to avoid overwriting data.</p>";
